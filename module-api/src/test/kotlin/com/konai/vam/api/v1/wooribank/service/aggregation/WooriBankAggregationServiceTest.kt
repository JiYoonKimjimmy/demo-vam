...
class WooriBankAggregationServiceTest : CustomBehaviorSpec({
    ...
    given("'20240628' 일자 우리은행 거래내역 집계 동기화 요청을 하여") {
        val aggegateDate = "20240628"

        `when`("요청 일자의 집계 Cache 정보가 없는 경우") {
            every { mockNumberRedisTemplate.opsForValue().multiGet(any()) } returns emptyList()

            val exception = shouldThrow<ResourceNotFoundException> { wooriBankAggregationService.aggregateTransaction(aggegateDate) }
        ...
        `when`("요청 일자의 집계 Cache 정보 기준 우리은행 집계 조회 요청 결과 실패인 경우") {
            ...
            shouldThrow<RestClientServiceException> { wooriBankAggregationService.aggregateTransaction(aggegateDate) }

            then("우리은행 집계 Cache 정보 조회 성공한다") {
                val cache = wooriBankAggregationCacheService.findAggregationCache(aggegateDate)
            ...
            then("우리은행 집계 'WATTING' 상태 정보 저장 성공한다") {
                val entity = wooriBankAggregationEntityAdapter.findByAggregateDate(aggegateDate)
                entity.aggregateResult shouldBe WAITING
            }
        }

        `when`("요청 일자의 집계 Cache 정보 기준 우리은행 집계 조회 요청 결과 집계 데이터가 일치하지 않은 경우") {
            val response = wooriBankRestClientModelFixture.makePostWooriAggregateTransactionResponse(aggegateDate)
            every { mockWooriBankRestClient.postWooriAggregateTransaction(any()) } returns response

            val result = wooriBankAggregationService.aggregateTransaction(aggegateDate)
        ...
        val keys = WooriBankAggregationCache(aggegateDate).keys
